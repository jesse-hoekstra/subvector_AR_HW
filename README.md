...
- Simulates joint eigenvalues \((\kappa{p}, \lambda_{p-1})\) for a given dimension `p` and number of instruments `k`, with noncentrality specified by `mu`.
...
```
Enter a p>2: 5
Enter the number of instruments n: 20
```
...
- **n**: integer > p (degrees of freedom / number of samples)
